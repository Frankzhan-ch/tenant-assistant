...
def generate_icalendar(reminder: dict) -> bytes:
    ...
    for days_before in sorted(reminder['reminder_days']):
        ...
        start_date = datetime(now.year, now.month, int(reminder['due_date']))
        if days_before > 0:
...
def show_rent_reminder_form():
    ...
    contract_info = {"amount": None, "due_date": None}
    if st.session_state.vectorstore is not None:
        docs = st.session_state.vectorstore.similarity_search("rent payment amount due date", k=3)
        contract_info = extract_rent_info(" ".join([d.page_content for d in docs]))
