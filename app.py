...
def send_email(to_email: str, subject: str, body_html: str) -> bool:
    ...
    if not (SMTP_SERVER and SMTP_PORT and SMTP_FROM and SMTP_PASS):
        st.warning("⚠️ SMTP not configured. Check environment variables: SMTP_SERVER, SMTP_PORT, SMTP_FROM, SMTP_PASS")
        return False
...
# =============================================================================
# 📅 RENT REMINDER HELPER FUNCTIONS
# =============================================================================
def extract_rent_info(user_message: str) -> dict:
    """
    Extract rent information from natural language input using regex patterns.
    Returns a dict with extracted information or empty dict if nothing found.
    """
    extracted = {}
    
    # Pattern for rent amount - try multiple patterns in order
    rent_patterns = [
        # Pattern 1: $2500, $2,500, $2500.00 (with dollar sign)
        (r'\$(\d+(?:,\d{3})*(?:\.\d{2})?)', True),
        # Pattern 2: 2500 dollars, 3000 per month (number with keywords)
        (r'(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:dollars?)\b', False),
        # Pattern 3: rent is 2500, rent: 3000
        (r'(?:rent(?:\s+is)?)\s*[:=]?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)', False),
        # Pattern 4: pay 2500, paying 3000
        (r'(?:pay(?:ing)?)\s+(\d+(?:,\d{3})*(?:\.\d{2})?)', False),
    ]
    
    for pattern, requires_dollar in rent_patterns:
        rent_match = re.search(pattern, user_message, re.IGNORECASE)
        if rent_match:
            amount_str = rent_match.group(1).replace(',', '')
            try:
                amount = float(amount_str)
                # Only accept amounts that make sense as rent (100-100000)
                if 100 <= amount <= 100000:
                    extracted['rent_amount'] = amount
                    break
            except ValueError:
                continue
    
    # Pattern for due date: "on the 1st", "due on 15th", "on day 20"
    due_date_patterns = [
        r'(?:on\s+(?:the\s+)?|due\s+on\s+(?:the\s+)?|day\s+)(\d{1,2})(?:st|nd|rd|th)?',
        r'due\s+(?:date|day)[\s:]+(\d{1,2})',
    ]
    for pattern in due_date_patterns:
        due_match = re.search(pattern, user_message, re.IGNORECASE)
        if due_match:
            try:
                day = int(due_match.group(1))
                if 1 <= day <= 31:
                    extracted['due_date'] = day
                    break
            except ValueError:
                pass
...
def generate_icalendar(reminder: dict) -> bytes:
    """
    Generate an iCalendar (.ics) file for a rent reminder.
    Creates recurring events for the specified duration.
    """
    if not ICALENDAR_AVAILABLE:
        return None
    
    cal = Calendar()
    cal.add('prodid', '-//Tenancy Assistant//Rent Reminder//EN')
    cal.add('version', '2.0')
    cal.add('calscale', 'GREGORIAN')
    cal.add('method', 'PUBLISH')
    cal.add('x-wr-calname', f"Rent Reminders - {reminder['property_address'][:30]}")
    cal.add('x-wr-timezone', 'UTC')
    
    # Calculate start and end dates
    now = datetime.now()
    duration_months = int(reminder.get('reminder_duration', 12))
    
    # For each reminder day (e.g., 3 days before, 1 day before, due date)
    for days_before in sorted(reminder['reminder_days']):
        try:
            # Start from next occurrence
            due_day = int(reminder['due_date'])
            
            # Handle invalid dates (e.g., Feb 30)
            try:
                start_date = datetime(now.year, now.month, due_day)
            except ValueError:
                # If current month doesn't have this day, use next month
                if now.month == 12:
                    start_date = datetime(now.year + 1, 1, min(due_day, 28))
                else:
                    # Find the last valid day of next month
                    next_month = now.month + 1
                    import calendar
                    max_day = calendar.monthrange(now.year, next_month)[1]
                    start_date = datetime(now.year, next_month, min(due_day, max_day))
            
            if days_before > 0:
                start_date = start_date - timedelta(days=days_before)
            
            # Adjust if the date has passed this month
            if start_date < now:
            ...
            # If we can't calculate this date, skip this reminder day
            continue
        
        # Calculate end date (after duration_months)
        end_year = start_date.year + (start_date.month + duration_months - 1) // 12
        end_month = (start_date.month + duration_months - 1) % 12 + 1
        end_date = datetime(end_year, end_month, start_date.day)
        
        # Create event
        event = Event()
        
        # Unique ID for this event
        event_id = f"{reminder['id']}-{days_before}days@tenancy-assistant"
        event.add('uid', event_id)
        
        # Event title
        if days_before == 0:
            title = f"💰 Rent Due - ${reminder['rent_amount']}"
        else:
            title = f"⏰ Rent Reminder ({days_before} days) - ${reminder['rent_amount']}"
        event.add('summary', title)
        
        # Event description with all details
        description_lines = [
            f"Property: {reminder['property_address']}",
            f"Amount: ${reminder['rent_amount']}",
            f"Due Date: Day {reminder['due_date']} of each month",
            f"",
        ]
        if reminder.get('landlord_info'):
            description_lines.append("Landlord/Payment Info:")
            description_lines.append(reminder['landlord_info'])
            description_lines.append("")
        if reminder.get('notes'):
            description_lines.append("Notes:")
            description_lines.append(reminder['notes'])
            description_lines.append("")
        description_lines.append(f"Reminder created: {reminder['created_at']}")
        description_lines.append(f"Email: {reminder['email']}")
        
        event.add('description', '\n'.join(description_lines))
        
        # Location (property address)
        event.add('location', reminder['property_address'])
        
        # All-day event
        event.add('dtstart', start_date.date())
        
        # Recurrence rule: monthly for the duration
        event.add('rrule', {'freq': 'monthly', 'count': duration_months})
        
        # Status and categories
        event.add('status', 'CONFIRMED')
        event.add('categories', ['RENT', 'PAYMENT', 'REMINDER'])
        
        # Priority based on urgency
        if days_before == 0:
            event.add('priority', 5)  # High priority on due date
        else:
            event.add('priority', 6)  # Medium priority for reminders
        
        # Alarm/Reminder at 9 AM on the event day
        from icalendar import Alarm
        alarm = Alarm()
        alarm.add('action', 'DISPLAY')
        alarm.add('description', title)
        alarm.add('trigger', timedelta(hours=-9))  # 9 AM reminder
...
def show_rent_reminder_form():
    ...
    # Auto-detection from user's last message
    auto_detected = {}
    if st.session_state.chat_history:
        ...
        if last_user_msg:
            auto_detected = extract_rent_info(last_user_msg)
            if auto_detected:
                st.info(f"🤖 Auto-detected from your message: " + 
                       (f"Rent: ${auto_detected.get('rent_amount', 'N/A')}, " if 'rent_amount' in auto_detected else "") +
                       (f"Due Date: Day {auto_detected.get('due_date', 'N/A')}" if 'due_date' in auto_detected else ""))

    # Also try to extract from contract if available
    contract_info = {}
    if st.session_state.vectorstore is not None:
        try:
            ...
            if docs:
                combined_text = " ".join([d.page_content for d in docs])
                contract_info = extract_rent_info(combined_text)
        except Exception:
            # If contract extraction fails, just continue with empty contract_info
            pass

    # Prioritize auto-detected values over contract values
    default_amount = auto_detected.get('rent_amount') or contract_info.get('rent_amount') or 0.0
    default_due = auto_detected.get('due_date') or contract_info.get('due_date') or 1
    ...
    with st.expander(f"{fav_emoji} {prop[1][:60]}... - ${monthly_price:,.0f}/month", expanded=False):
        ...
        with col_info:
            ...
            if host_info:
                ...
                button_label = "🔍 Hide Host Details" if st.session_state[host_key] else "🔍 View Host Details"
            ...
            with st.form(key=f"form_{prop_id}_{key_suffix}"):
                ...
                if submitted:
                    if name and email and date:
                        ...
                        st.info("📧 Confirmation email sent to your address.")
                        st.session_state[f"booking_open_{prop_id}_{key_suffix}"] = False
                        st.balloons()
                        st.rerun()
                    else:
    ...
    for msg in st.session_state.chat_history:
        ...
        if role == "user":
            ...
            if "rouge_scores" in msg and msg["rouge_scores"] and "error" not in msg["rouge_scores"]:
                with st.expander("📊 Answer Quality Metric (ROUGE-L Precision)", expanded=False):
                    ...
                    if rouge_l_precision > 0.6:
                        ...
                        st.info(f"🟡 **ROUGE-L Precision: {rouge_l_precision:.4f}** (Medium)")
                        st.caption("Answer partially uses source text")
                    else:
                        st.warning(f"⚠️ **ROUGE-L Precision: {rouge_l_precision:.4f}** (Low)")
                        st.caption("Answer may not be well-grounded - please verify")
                    
                    st.markdown('</div>', unsafe_allow_html=True)
                    st.caption("💡 ROUGE-L Precision measures the longest common subsequence overlap between the answer and source text. Higher scores indicate better grounding in retrieved context.")
...
with tab2:
                    ...
                    if req['photos']:
                        ...
                        for idx, photo_name in enumerate(req['photos']):
                            with cols[idx % 4]:
                                ...
                                button_label = "🖼️ Hide Photo" if is_showing else f"🖼️ {photo_name}"
                                ...
                                if st.session_state[f"show_photo_{req['id']}"].get(photo_key, False):
                                    try:
                                        ...
                                        st.warning(f"⚠️ Cannot display: {photo_name}")
    ...
    with subtab4:
        ...
        if not st.session_state.rent_reminders:
            ...
            for i, reminder in enumerate(st.session_state.rent_reminders):
                ...
                with st.expander(
                    ...
                    if ICALENDAR_AVAILABLE:
                        if st.button("📅 Download iCalendar (.ics)", key=f"ical_{reminder['id']}", use_container_width=True):
                            ics_data = generate_icalendar(reminder)
                            if ics_data:
                                st.download_button(
                                    label="⬇️ Download Calendar File",
                                    data=ics_data,
                                    file_name=f"rent_reminder_{reminder['id']}.ics",
                                    mime="text/calendar",
                                    key=f"ical_download_{reminder['id']}",
                                    use_container_width=True
                                )
                                st.success("✅ Click above to download the .ics file and import it into your calendar app!")
                                st.info("📱 Import this file into Google Calendar, Outlook, Apple Calendar, or any other calendar application.")
                            else:
                                st.error("Failed to generate iCalendar file")
                    else:
                        st.warning("📅 iCalendar export requires 'icalendar' package. Install with: pip install icalendar")
            ...
            with col_ical:
                if ICALENDAR_AVAILABLE:
                    if st.button("📅 Export All to iCalendar", use_container_width=True):
                        # Create a combined calendar with all reminders
                        combined_cal = Calendar()
                        combined_cal.add('prodid', '-//Tenancy Assistant//All Rent Reminders//EN')
                        combined_cal.add('version', '2.0')
                        combined_cal.add('calscale', 'GREGORIAN')
                        combined_cal.add('method', 'PUBLISH')
                        combined_cal.add('x-wr-calname', 'All Rent Reminders')
                        combined_cal.add('x-wr-timezone', 'UTC')
                        
                        for reminder in st.session_state.rent_reminders:
                            # Generate events for this reminder and add to combined calendar
                            single_cal_bytes = generate_icalendar(reminder)
                            if single_cal_bytes:
                                # Parse the generated calendar and extract events
                                from icalendar import Calendar as iCal
                                single_cal = iCal.from_ical(single_cal_bytes)
                                for component in single_cal.walk():
                                    if component.name == "VEVENT":
                                        combined_cal.add_component(component)
                        
                        combined_ics = combined_cal.to_ical()
                        st.download_button(
                            "⬇️ Download Combined Calendar",
                            combined_ics,
                            "all_rent_reminders.ics",
                            "text/calendar",
                            use_container_width=True
                        )
                        st.success("✅ Click above to download all reminders as one calendar file!")
                else:
                    st.info("📅 Install 'icalendar' package for calendar export")
